...
Generally, MsQuic creates multiple threads to parallelize work, and therefore will make overlapping upcalls to the application, but not for the same connection. All upcalls to the app for a single connection and all child streams area always delivered synchronously. This is not to say, though, it will always be on the same thread. MsQuic does support the ability to shuffle connections around to better balance the load.
