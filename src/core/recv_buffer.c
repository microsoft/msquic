    ...
    )
{
    ...
    if (PreallocatedBuffer != NULL) {
        ...
        RecvBuffer->Buffer = CXPLAT_ALLOC_NONPAGED(AllocBufferLength, CXPLAT_POOL_RECVBUF);
    ...
    )
{
    ...
    if (RecvBuffer->Buffer != RecvBuffer->PreallocatedBuffer) {
        CXPLAT_FREE(RecvBuffer->Buffer, CXPLAT_POOL_RECVBUF);
    }
    RecvBuffer->Buffer = NULL;
    if (RecvBuffer->OldBuffer != NULL &&
        RecvBuffer->OldBuffer != RecvBuffer->PreallocatedBuffer) {
        CXPLAT_FREE(RecvBuffer->OldBuffer, CXPLAT_POOL_RECVBUF);
    }
    RecvBuffer->OldBuffer = NULL;
}
    ...
    )
{
    ...
    if (TargetBufferLength != RecvBuffer->AllocBufferLength) {
        ...
        uint8_t* NewBuffer = CXPLAT_ALLOC_NONPAGED(TargetBufferLength, CXPLAT_POOL_RECVBUF);
        if (NewBuffer == NULL) {
            Status = QUIC_STATUS_OUT_OF_MEMORY;
            goto Error;
        }
