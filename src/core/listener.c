...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
    ...
    Status =
        QuicLibraryGetBinding(
            ...
            &Listener->Binding);
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(ListenerErrorStatus, "[list][%p] ERROR, %u, %s.", Listener, Status, "Get binding");
        goto Error;
    }
    ...
    )
{
    ...
    if (Status == QUIC_STATUS_PENDING) {
        ...
        *SecConfig = NULL;
        ...
        QuicTraceEvent(ListenerErrorStatus, "[list][%p] ERROR, %u, %s.", Listener, Status, "NEW_CONNECTION callback");
        goto Exit;
