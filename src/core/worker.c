    ...
    )
{
    ...
    QuicTraceEvent(WorkerCreated, "[wrkr][%p] Created, IdealProc=%hhu Owner=%p", Worker, IdealProcessor, Owner);
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(WorkerErrorStatus, "[wrkr][%p] ERROR, %u, %s.", Worker, Status, "QuicThreadCreate");
...
{
    QuicTraceEvent(WorkerCleanup, "[wrkr][%p] Cleaning up", Worker);
    ...
    QuicTraceEvent(WorkerDestroyed, "[wrkr][%p] Destroyed", Worker);
}
    ...
    )
{
    ...
    QuicTraceEvent(ConnAssignWorker, "[conn][%p] Assigned worker: %p", Connection, Worker);
}
    ...
    )
{
    ...
    if (!Connection->WorkerProcessing && !Connection->HasQueuedWork) {
        ...
        QuicTraceEvent(ConnScheduleState, "[conn][%p] Scheduling: %u", Connection, QUIC_SCHEDULE_QUEUED);
    ...
    )
{
    ...
    QuicTraceEvent(WorkerActivityStateUpdated, "[wrkr][%p] IsActive = %hhu, Arg = %u", Worker, Worker->IsActive, Arg);
}
    ...
    )
{
    ...
    QuicTraceEvent(WorkerQueueDelayUpdated, "[wrkr][%p] QueueDelay = %u", Worker, Worker->AverageQueueDelay);
}
...
{
    QuicTraceEvent(ConnScheduleState, "[conn][%p] Scheduling: %u", Connection, QUIC_SCHEDULE_PROCESSING);
    QuicSessionAttachSilo(Connection->Session);
    ...
    if (!Connection->State.UpdateWorker) {
        if (Connection->HasQueuedWork) {
            ...
            QuicTraceEvent(ConnScheduleState, "[conn][%p] Scheduling: %u", Connection, QUIC_SCHEDULE_IDLE);
        }
    }
    QuicDispatchLockRelease(&Worker->Lock);
...
QUIC_THREAD_CALLBACK(QuicWorkerThread, Context)
{
    ...
    QuicTraceEvent(WorkerStart, "[wrkr][%p] Start", Worker);
    ...
    QuicTraceEvent(WorkerStop, "[wrkr][%p] Stop", Worker);
