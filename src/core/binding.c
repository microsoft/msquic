    ...
    )
{
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(BindingErrorStatus, "[bind][%p] ERROR, %u, %s.", Binding, Status, "Create reset token hash");
        goto Error;
    }
    ...
    if (PrevCompartmentId != CompartmentId) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(BindingErrorStatus, "[bind][%p] ERROR, %u, %s.", Binding, Status, "Set current compartment Id");
            goto Error;
        }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(BindingErrorStatus, "[bind][%p] ERROR, %u, %s.", Binding, Status, "Create datapath binding");
        goto Error;
    }
