    ...
    )
{
    ...
    case QUIC_PARAM_CONN_LOCAL_ADDRESS: {
        ...
        QuicTraceEvent(
            ...
            "[conn][%p] New Local IP: %!SOCKADDR!",
            Connection,
            CLOG_BYTEARRAY(LOG_ADDR_LEN(Connection->Paths[0].LocalAddress), (const uint8_t*)&Connection->Paths[0].LocalAddress));
