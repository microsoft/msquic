    ...
    )
{
    ...
    QuicTraceEvent(ConnCreated, "[conn][%p] Created, IsServer=%hu, CorrelationId=%llu", Connection, IsServer, Connection->Stats.CorrelationId);
    ...
    if (IsServer) {
        ...
        QuicTraceEvent(ConnDestCidAdded, "[conn][%p] (SeqNum=%llu) New Destination CID: %!CID!",
        ...
        QuicTraceEvent(ConnSourceCidAdded, "[conn][%p] (SeqNum=%llu) New Source CID: %!CID!",
...
{
    QuicTraceEvent(ConnRundown, "[conn][%p] Rundown, IsServer=%hu, CorrelationId=%llu",
        Connection,
        QuicConnIsServer(Connection),
        Connection->Stats.CorrelationId);
    ...
    )
{
    ...
    do {
        ...
        if (SourceCid == NULL) {
            QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)", "new Src CID", sizeof(QUIC_CID_HASH_ENTRY) + MsQuicLib.CidTotalLength);
    ...
    QuicTraceEvent(ConnSourceCidAdded, "[conn][%p] (SeqNum=%llu) New Source CID: %!CID!", Connection, SourceCid->CID.SequenceNumber, SourceCid->CID.Length, SourceCid->CID.Data);
...
{
    QuicTraceEvent(ConnDestCidRemoved, "[conn][%p] (SeqNum=%llu) Removed Destination CID: %!CID!", Connection, DestCid->CID.SequenceNumber, DestCid->CID.Length, DestCid->CID.Data);
    Connection->DestCidCount--;
    ...
    )
{
    ...
    for (uint32_t j = 0; j < i; ++j) {
        ...
        if (Temp[j].Type == QUIC_CONN_TIMER_ACK_DELAY) {
            QuicTraceEvent(ConnExecTimerOper, "[conn][%p] Execute: %u", Connection, QUIC_CONN_TIMER_ACK_DELAY);
            QuicSendProcessDelayedAckTimer(&Connection->Send);
            FlushSendImmediate = TRUE;
        } else if (Temp[j].Type == QUIC_CONN_TIMER_PACING) {
            QuicTraceEvent(ConnExecTimerOper, "[conn][%p] Execute: %u", Connection, QUIC_CONN_TIMER_PACING);
            FlushSendImmediate = TRUE;
        } else {
            QUIC_OPERATION* Oper;
    ...
    )
{
    ...
    QuicTraceEvent(ConnShutdownComplete, "[conn][%p] Shutdown complete, PeerFailedToAcknowledged=%hhu.",
    ...
    )
{
    ...
    if (IsFirstCloseForConnection) {
        ...
        if (Flags & QUIC_CLOSE_APPLICATION) {
            QuicTraceEvent(ConnAppShutdown, "[conn][%p] App Shutdown: %llu (Remote=%hhu)",
                Connection,
                ErrorCode,
                ClosedRemotely);
        } else {
            QuicTraceEvent(ConnTransportShutdown, "[conn][%p] Transport Shutdown: %llu (Remote=%hhu) (QS=%hhu)",
                Connection,
                ErrorCode,
                ClosedRemotely,
...
{
    QuicTraceEvent(ConnVersionSet, "[conn][%p] Version = %u", Connection, Connection->Stats.QuicVersion);
    ...
    )
{
    ...
    if (!Connection->State.RemoteAddressSet) {
        ...
        if (PrevCompartmentId != Connection->Session->CompartmentId) {
            ...
            if (QUIC_FAILED(Status)) {
                QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %u, %s.", Connection, Status, "Set current compartment Id");
                goto Exit;
            }
    ...
    )
{
    ...
    if (QuicConnIsServer(Connection)) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %u, %s.", Connection, Status,
                "QuicBindingGenerateStatelessResetToken");
            goto Error;
        }
        ...
        if (SecConfig == NULL) {
            Status =
                QuicTlsClientSecConfigCreate(
                    ...
                    &SecConfig);
            if (QUIC_FAILED(Status)) {
                QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %u, %s.", Connection, Status, "QuicTlsClientSecConfigCreate");
                goto Error;
            }
        }
    ...
    )
{
    ...
    if (Packet->SourceCidLen != DestCid->CID.Length ||
        ...
        if (DestCid != NULL) {
            QuicTraceEvent(ConnDestCidAdded, "[conn][%p] (SeqNum=%llu) New Destination CID: %!CID!", Connection, DestCid->CID.SequenceNumber, DestCid->CID.Length, DestCid->CID.Data);
        }
    }
    ...
    )
{
    ...
    while (Offset < PayloadLength) {
        ...
        if (EncryptLevel != QUIC_ENCRYPT_LEVEL_1_RTT) {
            ...
            //
            default:
                QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %u, %s.", Connection, FrameType, "Disallowed frame type");
                QuicConnTransportError(Connection, QUIC_ERROR_FRAME_ENCODING_ERROR);
                return FALSE;
            }
            ...
            if (SourceCid->CID.IsInitial) {
                if (QuicConnIsServer(Connection) && SourceCid->Link.Next != NULL) {
                    ...
                    if (NextSourceCid->CID.IsInitial) {
                        ...
                        QuicTraceEvent(ConnSourceCidRemoved, "[conn][%p] (SeqNum=%llu) Removed Source CID: %!CID!",
    ...
    )
{
    ...
    case QUIC_PARAM_CONN_FORCE_KEY_UPDATE:
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %u, %s.", Connection, Status, "Forced key update");
            break;
        }
