    ...
    )
{
    ...
    if (QuicConnIsClient(Connection)) {
        ...
        const uint8_t* AlpnList = Connection->Configuration->AlpnList;
        uint16_t AlpnListLength = Connection->Configuration->AlpnListLength;
        ...
        while (AlpnListLength != 0) {
            const uint8_t* Result =
                CxPlatTlsAlpnFindInList(
                    Connection->ClientAlpnListLength,
                    Connection->ClientAlpnList,
                    AlpnList[0],
                    AlpnList + 1);
            if (Result != NULL) {
                break;
        ...
        const uint8_t* NewNegotiatedAlpn = AlpnList;
