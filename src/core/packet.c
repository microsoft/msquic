    ...
    )
{
    ...
    if (Packet->AssignedToConnection) {
        ...
        QuicTraceEvent(ConnDropPacket, "[conn][%p] DROP packet[%I] Dst=%SOCKADDR Src=%SOCKADDR Reason=%s.", 
            Owner, 
            Packet->PacketNumberSet ? UINT64_MAX : Packet->PacketNumber, CLOG_BYTEARRAY(LOG_ADDR_LEN(Datagram->Tuple->LocalAddress), S2C(&Datagram->Tuple->LocalAddress)), CLOG_BYTEARRAY(LOG_ADDR_LEN(Datagram->Tuple->RemoteAddress), S2C(&Datagram->Tuple->RemoteAddress)), 
            Reason);
    } else {
    ...
    )
{
    ...
    if (Packet->AssignedToConnection) {
        ...
        QuicTraceEvent(ConnDropPacketEx, "[conn][%p] DROP packet[%I] Value=%I Dst=%SOCKADDR Src=%SOCKADDR Reason=%s.", 
            ...
            Value, CLOG_BYTEARRAY(LOG_ADDR_LEN(Datagram->Tuple->LocalAddress), (uint8_t*)&(Datagram->Tuple->LocalAddress)), CLOG_BYTEARRAY(LOG_ADDR_LEN(Datagram->Tuple->RemoteAddress), S2C(&Datagram->Tuple->RemoteAddress)), 
            Reason);
    } else {
        InterlockedIncrement64((int64_t*) &((QUIC_BINDING*)Owner)->Stats.Recv.DroppedPackets);
