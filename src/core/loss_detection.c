    ...
    )
{
    ...
    QuicTraceEvent(ConnLossDetectionTimerSet, "[conn][%p] Setting loss detection %hhu timer for %u ms. (ProbeCount=%hu)",
        ...
        while (Packet != NULL) {
            ...
            if (EncryptLevel > LossDetection->LargestAckEncryptLevel) {
                ...
                if (!NonretransmittableHandshakePacket) {
                    ...
                    QuicTraceEvent(ConnPacketLost, "[conn][%p][TX][%llu] %hhu Lost: %hhu",
                        Connection,
                        Packet->PacketNumber,
    ...
    while (Packet != NULL) {
        ...
        if (Packet->Flags.KeyType == KeyType) {
            ...
            QuicTraceLogVerbose(
                ...
                PtkConnPre(Connection),
                Packet->PacketNumber);
            QuicTraceEvent(ConnPacketACKed, "[conn][%p][TX][%llu] %hhu ACKed",
                Connection,
                Packet->PacketNumber,
                QuicPacketTraceType(Packet));
