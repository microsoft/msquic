...
#ifdef QUIC_CLOG
#include "LatencyClient.cpp.clog.h"
#endif
...
LatencyClient::Start(
    ...
    for (uint32_t i = 0; i < ConnectionCount; ++i) {
        ...
        ConnectionContext* ConnContext = ConnectionContextAllocator.Alloc(this);

        if (ConnContext == nullptr) {
            WriteOutput("Failed to allocate Connection Context\n");
            return QUIC_STATUS_OUT_OF_MEMORY;
        }
...
QUIC_STATUS
LatencyClient::ConnectionCallback(
    ...
    )
{
    ...
    if (ConnContext->IsInitialStream) {
        ConnContext->IsInitialStream = FALSE;
        ConnContext->OutstandingCount = 0;

        for (uint32_t i = 0; i < AfterStreamCount; i++) {
            ConnContext->OutstandingCount++;

            uint64_t StartTime = QuicTimeUs64();
            StreamContext* StrmContext = StreamContextAllocator.Alloc(this, ConnContext, StartTime);
    ...
    } else if (ConnContext->OutstandingCount == 0) {
        ConnContext->IsInitialStream = TRUE;
        ConnContext->OutstandingCount = 1;
