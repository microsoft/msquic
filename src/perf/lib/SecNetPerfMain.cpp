...
PrintHelp(
    ...
    WriteOutput(
        ...
        "  -upload:<####>[unit]     The length of bytes to send on each stream, with an option (time or length) unit. (def:0)\n"
        "  -download:<####>[unit]   The length of bytes to receive on each stream, with an option (time or length) unit. (def:0)\n"
