    ...
    )
{
    ...
    if (QUIC_SUCCEEDED(
        ApiTable->GetParam(
            ...
            &Statistics))) {
        WriteOutput(
            "[conn][%p] STATS: EcnCapable=%hu RTT=%u us SendTotalPackets=%llu SendSuspectedLostPackets=%llu SendSpuriousLostPackets=%llu SendCongestionCount=%u SendEcnCongestionCount=%lu RecvTotalPackets=%llu RecvReorderedPackets=%llu RecvDroppedPackets=%llu RecvDuplicatePackets=%llu RecvDecryptionFailures=%llu\n",
            Connection,
