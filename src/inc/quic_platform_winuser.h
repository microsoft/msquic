...
#define CXPLAT_LOOKASIDE_MAXIMUM_DEPTH 4096

typedef struct CXPLAT_LOOKASIDE CXPLAT_LOOKASIDE;

typedef
void*
(*CXPLAT_LOOKASIDE_ALLOC_FN)(
    _In_ BOOLEAN IsPaged,
    _In_ uint32_t Size,
    _In_ uint32_t Tag,
...
typedef
void
(*CXPLAT_LOOKASIDE_FREE_FN)(
    _In_ void* Entry,
    _In_ uint32_t Tag,
    _Inout_ CXPLAT_LOOKASIDE* Lookaside
    );

typedef struct CXPLAT_LOOKASIDE {
    SLIST_HEADER ListHead;
    uint32_t Size;
    uint32_t Tag;
    uint32_t MaxDepth;
    CXPLAT_LOOKASIDE_ALLOC_FN Allocate;
    CXPLAT_LOOKASIDE_FREE_FN Free;
} CXPLAT_LOOKASIDE;

typedef struct CXPLAT_LOOKASIDE_ENTRY {
    SLIST_ENTRY ListEntry;
} CXPLAT_LOOKASIDE_ENTRY;

inline
void*
CxPlatLookasideGenericAlloc(
    _In_ BOOLEAN IsPaged,
    _In_ uint32_t Size,
    _In_ uint32_t Tag,
    _Inout_ CXPLAT_LOOKASIDE* Lookaside
...
CxPlatLookasideInitialize(
    ...
    _In_ uint32_t Tag,
    _In_ uint32_t MaxDepth,
    _In_opt_ CXPLAT_LOOKASIDE_ALLOC_FN Allocate,
    _In_opt_ CXPLAT_LOOKASIDE_FREE_FN Free,
    _Inout_ CXPLAT_LOOKASIDE* Lookaside
    )
{
    CXPLAT_DBG_ASSERT(Size >= sizeof(CXPLAT_LOOKASIDE_ENTRY));
    Lookaside->Size = Size;
    Lookaside->Tag = Tag;
    Lookaside->Allocate = Allocate ? Allocate : CxPlatLookasideGenericAlloc;
    Lookaside->Free = Free ? Free : CxPlatLookasideGenericFree;
    InitializeSListHead(&(Lookaside)->ListHead);
    UNREFERENCED_PARAMETER(IsPaged);

    if (Lookaside->MaxDepth != 0) {
        Lookaside->MaxDepth = CXPLAT_MIN(MaxDepth, CXPLAT_LOOKASIDE_MAXIMUM_DEPTH);
    } else {
        Lookaside->MaxDepth = CXPLAT_POOL_MAXIMUM_DEPTH;
    }
}

inline
void
CxPlatLookasideUninitialize(
    _Inout_ CXPLAT_LOOKASIDE* Lookaside
    )
{
    void* Entry;
    while ((Entry = InterlockedPopEntrySList(&Lookaside->ListHead)) != NULL) {
        Lookaside->Free(Entry, Lookaside->Tag, Lookaside);
    }
}

inline
void*
CxPlatLookasideAlloc(
    _Inout_ CXPLAT_LOOKASIDE* Lookaside
    )
{
    void* Entry = InterlockedPopEntrySList(&Lookaside->ListHead);
    if (Entry == NULL) {
        Entry = Lookaside->Allocate(FALSE, Lookaside->Size, Lookaside->Tag, Lookaside);
    }
    return Entry;
}
...
{
    if (QueryDepthSList(&Lookaside->ListHead) >= Lookaside->MaxDepth) {
        Lookaside->Free(Entry, Lookaside->Tag, Lookaside);
    } else {
        InterlockedPushEntrySList(
            &Lookaside->ListHead, &((CXPLAT_LOOKASIDE_ENTRY*)Entry)->ListEntry);
    }
}
