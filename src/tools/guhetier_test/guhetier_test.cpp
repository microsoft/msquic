...
//
const QUIC_REGISTRATION_CONFIG RegConfig = { "guhetier_test", QUIC_EXECUTION_PROFILE_LOW_LATENCY };

//
// The protocol name used in the Application Layer Protocol Negotiation (ALPN).
//
const QUIC_BUFFER Alpn = { sizeof("guhetier_test") - 1, (uint8_t*)"guhetier_test" };
...
//
// The name of the environment variable being used to get the path to the ssl key log file.
// Useful to get the TLS secrets to decrypt QUIC packets in Wireshark.
//
    ...
    )
{
    ...
    case QUIC_LISTENER_EVENT_NEW_CONNECTION:
        ...
        //
        printf("[list][%p] New connection %p\n", Listener, Event->NEW_CONNECTION.Connection);
    ...
    )
{
    ...
    //
    Status = MsQuic->ListenerOpen(Registration, ServerListenerCallback, NULL, &Listener);
    ...
    )
{
    ...
    //
    Status = MsQuic->ConnectionOpen(Registration, ClientConnectionCallback, NULL, &Connection);
    if (QUIC_FAILED(Status)) {
    ...
    //
    Status = MsQuic->ConnectionStart(Connection, Configuration, QUIC_ADDRESS_FAMILY_UNSPEC, Target, UdpPort);
    if (QUIC_FAILED(Status)) {
