...
public:
    ...
    void WriteSslKeyLogFile(const char* FileName) {
        ...
        char ClientRandomBuffer[64 + 1] = {0};
        char TempHexBuffer[96 + 1] = {0};
        if (SslKeyLog.IsSet.ClientRandom) {
            PrintHexBuffer(
                SslKeyLog.ClientRandom,
                sizeof(SslKeyLog.ClientRandom),
                ClientRandomBuffer);
        }

        if (SslKeyLog.IsSet.ClientEarlyTrafficSecret) {
            PrintHexBuffer(
                SslKeyLog.ClientEarlyTrafficSecret,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.ClientHandshakeTrafficSecret) {
            PrintHexBuffer(
                SslKeyLog.ClientHandshakeTrafficSecret,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.ServerHandshakeTrafficSecret) {
            PrintHexBuffer(
                SslKeyLog.ServerHandshakeTrafficSecret,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.ClientTrafficSecret0) {
            PrintHexBuffer(
                SslKeyLog.ClientTrafficSecret0,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.ServerTrafficSecret0) {
            PrintHexBuffer(
                SslKeyLog.ServerTrafficSecret0,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.EarlyExporterSecret) {
            PrintHexBuffer(
                SslKeyLog.EarlyExporterSecret,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
        ...
        if (SslKeyLog.IsSet.ExporterSecret) {
            PrintHexBuffer(
                SslKeyLog.ExporterSecret,
                SslKeyLog.SecretLength,
                TempHexBuffer);
            fprintf(
                File,
                "EXPORTER_SECRET_0 %s %s\n",
                ClientRandomBuffer,
                TempHexBuffer);
        }
