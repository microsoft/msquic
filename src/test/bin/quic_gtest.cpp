...
TEST_P(WithProbePathArgs, ProbePath) {
    TestLoggerT<ParamType> Logger("QuicTestProbePath_NoShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestProbePath_NoShareBinding), GetParam()));
    } else {
        QuicTestProbePath_NoShareBinding(GetParam());
    }
}

TEST_P(WithProbePathArgs, ProbePathShareBinding) {
    TestLoggerT<ParamType> Logger("QuicTestProbePath_WithShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestProbePath_WithShareBinding), GetParam()));
    } else {
        QuicTestProbePath_WithShareBinding(GetParam());
    }
}
...
INSTANTIATE_TEST_SUITE_P(
    Basic,
    WithProbePathArgs,
    ::testing::ValuesIn(WithProbePathArgs::Generate()));

TEST_P(WithFamilyArgs, ProbePathFailed) {
    TestLoggerT<ParamType> Logger("QuicTestProbePathFailed_NoShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestProbePathFailed_NoShareBinding), GetParam()));
    } else {
        QuicTestProbePathFailed_NoShareBinding(GetParam());
    }
}

TEST_P(WithFamilyArgs, ProbePathFailedShareBinding) {
    TestLoggerT<ParamType> Logger("QuicTestProbePathFailed_WithShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestProbePathFailed_WithShareBinding), GetParam()));
    } else {
        QuicTestProbePathFailed_WithShareBinding(GetParam());
    }
}

struct WithAddPathBeforeStartArgs :
    public testing::TestWithParam<AddPathBeforeStartArgs> {

    static ::std::vector<AddPathBeforeStartArgs> Generate() {
        ::std::vector<AddPathBeforeStartArgs> list;
        for (int Family : { 4, 6 })
        for (BOOLEAN DeferConnIDGen : { TRUE, FALSE })
            list.push_back({ Family, DeferConnIDGen });
        return list;
    }
};
...
TEST_P(WithAddPathBeforeStartArgs, AddPathBeforeStart) {
    TestLoggerT<ParamType> Logger("QuicTestAddPathBeforeStart_NoShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestAddPathBeforeStart_NoShareBinding), GetParam()));
    } else {
        QuicTestAddPathBeforeStart_NoShareBinding(GetParam());
    }
}

TEST_P(WithAddPathBeforeStartArgs, AddPathBeforeStartShareBinding) {
    TestLoggerT<ParamType> Logger("QuicTestAddPathBeforeStart_WithShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestAddPathBeforeStart_WithShareBinding), GetParam()));
    } else {
        QuicTestAddPathBeforeStart_WithShareBinding(GetParam());
    }
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(WithAddPathBeforeStartArgs::Generate()));

struct WithMigrationArgs :
    public testing::TestWithParam<MigrationArgs> {
...
TEST_P(WithMigrationArgs, Migration) {
    TestLoggerT<ParamType> Logger("QuicTestMigration_NoShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestMigration_NoShareBinding), GetParam()));
    } else {
        QuicTestMigration_NoShareBinding(GetParam());
    }
}

TEST_P(WithMigrationArgs, MigrationShareBinding) {
    TestLoggerT<ParamType> Logger("QuicTestMigration_WithShareBinding", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestMigration_WithShareBinding), GetParam()));
    } else {
        QuicTestMigration_WithShareBinding(GetParam());
    }
}
...
TEST_P(WithFamilyArgs, AddressDiscovery) {
    ...
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestAddressDiscovery), GetParam()));
    } else {
        QuicTestAddressDiscovery(GetParam());
    }

}

TEST_P(WithProbePathArgs, ServerProbePath) {
    TestLoggerT<ParamType> Logger("QuicTestServerProbePath", GetParam());
    if (TestingKernelMode) {
        ASSERT_TRUE(InvokeKernelTest(FUNC(QuicTestServerProbePath), GetParam()));
    } else {
        QuicTestServerProbePath(GetParam());
    }
}

TEST_P(WithMigrationArgs, ServerMigration) {
