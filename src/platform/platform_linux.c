    ...
    )
{
    ...
    if (pthread_attr_init(&Attr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", errno, "pthread_attr_init failed");
        return errno;
    }
    ...
    if (Config->Flags & QUIC_THREAD_FLAG_HIGH_PRIORITY) {
        ...
        if (!pthread_attr_setschedparam(&Attr, &Params)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", errno, "pthread_attr_setschedparam failed");
        }
    }

    if (pthread_create(Thread, &Attr, Config->Callback, Config->Context)) {
        Status = errno;
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Status, "pthread_create failed");
    }
