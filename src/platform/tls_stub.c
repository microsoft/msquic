    ...
    )
{
    ...
    case QUIC_TLS_MESSAGE_SERVER_HANDSHAKE: {
        if (ClientMessage->Type == QUIC_TLS_MESSAGE_CLIENT_HANDSHAKE) {
            ...
            if (ClientMessage->CLIENT_HANDSHAKE.Success == FALSE) {
                QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection, "Failure client finish");
                *ResultFlags |= QUIC_TLS_RESULT_ERROR;
                break;
            }
            ...
            *ResultFlags |= QUIC_TLS_RESULT_READ_KEY_UPDATED;
            ...
            QuicTraceEvent(TlsErrorStatus, "[ tls][%p] ERROR, %u, %s.", TlsContext->Connection, ClientMessage->Type, "Invalid message");
            *ResultFlags |= QUIC_TLS_RESULT_ERROR;
            break;
        }
    ...
    default: {
        QuicTraceEvent(TlsErrorStatus, "[ tls][%p] ERROR, %u, %s.", TlsContext->Connection, TlsContext->LastMessageType, "Invalid last message");
        *ResultFlags |= QUIC_TLS_RESULT_ERROR;
        break;
    }
    }
        ...
        if (ServerMessage->Type == QUIC_TLS_MESSAGE_SERVER_INITIAL) {
            ...
            *ResultFlags |= QUIC_TLS_RESULT_WRITE_KEY_UPDATED;
            ...
            if (TlsContext->SecConfig->Flags & QUIC_CERTIFICATE_FLAG_DISABLE_CERT_VALIDATION) {
                ...
                if (ServerCert == NULL) {
                    QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection, "QuicCertParseChain Mismatch");
                    *ResultFlags |= QUIC_TLS_RESULT_ERROR;
                    break;
                }
                ...
                if (!QuicCertValidateChain(
                    ...
                    QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection, "QuicCertValidateChain Mismatch");
                    *ResultFlags |= QUIC_TLS_RESULT_ERROR;
                    break;
                }
            }
            ...
            *ResultFlags |= QUIC_TLS_RESULT_WRITE_KEY_UPDATED;
            ...
            QuicTraceEvent(TlsErrorStatus, "[ tls][%p] ERROR, %u, %s.", TlsContext->Connection, ServerMessage->Type, "Invalid message");
            *ResultFlags |= QUIC_TLS_RESULT_ERROR;
            break;
        }
    ...
    )
{
    ...
    if (BufferLength < 7) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection, "Insufficient data to process header");
        return FALSE;
    }
    ...
    if (BufferLength < MessageLength) {
        QuicTraceEvent(TlsError, "[ tls][%p] ERROR, %s.", TlsContext->Connection, "Insufficient data to process payload");
        return FALSE;
    }
