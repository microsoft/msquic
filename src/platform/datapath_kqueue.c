...
QUIC_RECV_DATAGRAM* QuicDataPathRecvPacketToRecvDatagram(_In_ const QUIC_RECV_PACKET* const Context) {
    return (QUIC_RECV_DATAGRAM*) (((PUCHAR)Context) - sizeof(QUIC_DATAPATH_INTERNAL_RECV_BUFFER_CONTEXT) - sizeof(QUIC_RECV_DATAGRAM));
}

QUIC_RECV_PACKET* QuicDataPathRecvDatagramToRecvPacket(_In_ const QUIC_RECV_DATAGRAM* const Datagram) {
    return (QUIC_RECV_PACKET*) (((PUCHAR)Datagram) + sizeof(QUIC_RECV_DATAGRAM) + sizeof(QUIC_DATAPATH_INTERNAL_RECV_BUFFER_CONTEXT));
}
...
QUIC_DATAPATH_INTERNAL_RECV_BUFFER_CONTEXT* QuicDataPathDatagramToInternalDatagramContext(_In_ QUIC_RECV_DATAGRAM* Datagram) {
    return (QUIC_DATAPATH_INTERNAL_RECV_BUFFER_CONTEXT*) (((PUCHAR)Datagram) + sizeof(QUIC_RECV_DATAGRAM));
}

//
// Callback function for IOCP Worker Thread.
//
DWORD QuicDataPathWorkerThread(_In_ void* Context);
...
void QuicDataPathPopulateTargetAddress(_In_ ADDRESS_FAMILY Family, _In_ ADDRINFOW *Ai, _Out_ SOCKADDR_INET* Address) {
...
QUIC_STATUS QuicDataPathBindingStartReceive(_In_ QUIC_UDP_SOCKET_CONTEXT* SocketContext, _In_ HANDLE CompletionPort);
...
_IRQL_requires_max_(DISPATCH_LEVEL)
UINT16 QuicDataPathBindingGetLocalMtu(_In_ QUIC_DATAPATH_BINDING* Binding) {
    QUIC_DBG_ASSERT(Binding != NULL);
    return Binding->Mtu;
}

_IRQL_requires_max_(DISPATCH_LEVEL)
void QuicDataPathBindingGetLocalAddress(_In_ QUIC_DATAPATH_BINDING* Binding, _Out_ SOCKADDR_INET * Address) {
    QUIC_DBG_ASSERT(Binding != NULL);
    *Address = Binding->LocalAddress;
}

_IRQL_requires_max_(DISPATCH_LEVEL)
void QuicDataPathBindingGetRemoteAddress(_In_ QUIC_DATAPATH_BINDING* Binding, _Out_ SOCKADDR_INET * Address) {
    QUIC_DBG_ASSERT(Binding != NULL);
    *Address = Binding->RemoteAddress;
}

QUIC_DATAPATH_INTERNAL_RECV_CONTEXT* QuicDataPathBindingAllocRecvContext(_In_ QUIC_DATAPATH* Datapath, _In_ UINT16 ProcIndex) {
...
void QuicDataPathBindingHandleUnreachableError(_In_ QUIC_UDP_SOCKET_CONTEXT* SocketContext, _In_ ULONG ErrorCode) {
    PSOCKADDR_INET RemoteAddr = &SocketContext->CurrentRecvContext->Tuple.RemoteAddress;
...
QUIC_STATUS QuicDataPathBindingStartReceive(_In_ QUIC_UDP_SOCKET_CONTEXT* SocketContext, _In_ HANDLE CompletionPort) {
    ...
    //
    if (SocketContext->CurrentRecvContext == NULL) {
        SocketContext->CurrentRecvContext = QuicDataPathBindingAllocRecvContext(Datapath, (UINT16)GetCurrentProcessorNumber());
    ...
    RtlZeroMemory(&SocketContext->RecvOverlapped, sizeof(SocketContext->RecvOverlapped));

    SocketContext->RecvWsaBuf.buf = ((CHAR*)SocketContext->CurrentRecvContext) + Datapath->RecvPayloadOffset;

    RtlZeroMemory(&SocketContext->RecvWsaMsgHdr, sizeof(SocketContext->RecvWsaMsgHdr));

    SocketContext->RecvWsaMsgHdr.name = (PSOCKADDR)&SocketContext->CurrentRecvContext->Tuple.RemoteAddress;
    SocketContext->RecvWsaMsgHdr.namelen = sizeof(SocketContext->CurrentRecvContext->Tuple.RemoteAddress);
...
QUIC_DATAPATH_SEND_CONTEXT* QuicDataPathBindingAllocSendContext(_In_ QUIC_DATAPATH_BINDING* Binding, _In_ QUIC_ECN_TYPE ECN, _In_ uint16_t MaxPacketSize) {
    ...
    QUIC_DATAPATH_PROC_CONTEXT* ProcContext = &Binding->Datapath->ProcContexts[GetCurrentProcessorNumber()];
    ...
    if (SendContext != NULL) {
        ...
        SendContext->ECN = ECN;
        SendContext->SegmentSize = (Binding->Datapath->Features & QUIC_DATAPATH_FEATURE_SEND_SEGMENTATION) ? MaxPacketSize : 0;
...
static
BOOLEAN QuicSendContextCanAllocSendSegment(_In_ QUIC_DATAPATH_SEND_CONTEXT* SendContext, _In_ UINT16 MaxBufferLength) {
    ...
    ULONG BytesAvailable = QUIC_LARGE_SEND_BUFFER_SIZE - SendContext->WsaBuffers[SendContext->WsaBufferCount - 1].len - SendContext->ClientBuffer.len;
...
BOOLEAN QuicSendContextCanAllocSend(_In_ QUIC_DATAPATH_SEND_CONTEXT* SendContext, _In_ UINT16 MaxBufferLength) {
    return (SendContext->WsaBufferCount < SendContext->Owner->Datapath->MaxSendBatchSize) ||
...
void QuicSendContextFinalizeSendBuffer(_In_ QUIC_DATAPATH_SEND_CONTEXT* SendContext, _In_ BOOLEAN IsSendingImmediately) {
    if (SendContext->ClientBuffer.len == 0) {
        ...
        if (SendContext->WsaBufferCount > 0) {
            ...
            SendContext->TotalSize += SendContext->WsaBuffers[SendContext->WsaBufferCount - 1].len;
        }
        return;
    }
    ...
    //
    SendContext->WsaBuffers[SendContext->WsaBufferCount - 1].len += SendContext->ClientBuffer.len;
...
QUIC_STATUS QuicDataPathBindingSendTo(_In_ QUIC_DATAPATH_BINDING* Binding, _In_ const SOCKADDR_INET * RemoteAddress, _In_ QUIC_DATAPATH_SEND_CONTEXT* SendContext) 
{
    ...
    QuicTraceEvent(DatapathSendTo, "[ udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst=%!ADDR!", Binding, SendContext->TotalSize, SendContext->WsaBufferCount, SendContext->SegmentSize, CLOG_BYTEARRAY(sizeof(*RemoteAddress), RemoteAddress));
    ...
    )
{
    ...
    QuicTraceEvent(DatapathSendFromTo, "[ udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst=%!ADDR!, Src=%!ADDR!", Binding, SendContext->TotalSize, SendContext->WsaBufferCount, SendContext->SegmentSize, CLOG_BYTEARRAY(sizeof(*RemoteAddress), RemoteAddress), CLOG_BYTEARRAY(sizeof(*LocalAddress), LocalAddress));
...
DWORD QuicDataPathWorkerThread(_In_ void* CompletionContext) {
    ...
    while (TRUE) {
        ...
        if (Overlapped == &SocketContext->RecvOverlapped) {
            ...
            QUIC_DATAPATH_SEND_CONTEXT* SendContext = CONTAINING_RECORD(Overlapped, QUIC_DATAPATH_SEND_CONTEXT, Overlapped);

            QuicSendContextComplete(SocketContext, SendContext, IoResult);
        }
    }
