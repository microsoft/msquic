...
CleanTestCertificatesFromStore(BOOLEAN UserStore)
{
    ...
    HCERTSTORE CertStore =
        CertOpenStore(
            ...
            "MY");
    if (CertStore == NULL) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", GetLastError(), "CertOpenStore failed");
        return;
    }
    ...
    )
{
    ...
    if (!CryptEncodeObject(
        ...
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CryptEncodeObject failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    HRESULT hr =
        AllocateAndEncodeObject(
            ...
            &CertEnhKeyUsage);
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "AllocateAndEncodeObject X509_ENHANCED_KEY_USAGE failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    HRESULT hr =
        AllocateAndEncodeObject(
            ...
            &KeyUsageBlob);
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "AllocateAndEncodeObject X509_KEY_USAGE failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    HRESULT hr =
        AllocateAndEncodeObject(
            ...
            &NameInfo);
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "AllocateAndEncodeObject szOID_SUBJECT_ALT_NAME failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    if (!CertStrToNameW(
            ...
            NULL)) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CreateSubjectNameBlob failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CreateEnhancedKeyUsageCertExtension failed");
        goto Cleanup;
    }
    ...
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CreateKeyUsageCertExtension failed");
        goto Cleanup;
    }
    ...
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CreateSubjAltNameExtension failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    *Key = (NCRYPT_KEY_HANDLE)NULL;
    ...
    if (FAILED(hr = NCryptOpenStorageProvider(
            ...
            0))) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptOpenStorageProvider failed");
        goto Cleanup;
    }
...
ReadKey:
    ...
    if (hr == ERROR_SUCCESS) {
        ...
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptOpenKey failed");
        goto Cleanup;
    }
    ...
    if (hr == NTE_EXISTS) {
        ...
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptCreatePersistedKey failed");
        goto Cleanup;
    }
    ...
    if (FAILED(hr = NCryptSetProperty(
            *Key,
            ...
            sizeof(KeySize),
            0))) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptSetProperty NCRYPT_LENGTH_PROPERTY failed");
        goto Cleanup;
    }
    ...
    if (FAILED(hr = NCryptSetProperty(
            *Key,
            ...
            0))) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptSetProperty NCRYPT_KEY_USAGE_PROPERTY failed");
        goto Cleanup;
    }

    if (FAILED(hr = NCryptFinalizeKey(*Key, 0))) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "NCryptFinalizeKey failed");
        goto Cleanup;
    }
    ...
    )
{
    ...
    if (FAILED(hr)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "GetPrivateRsaKey failed");
        goto Cleanup;
    }
    ...
    if (FAILED(hr)) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CreateCertificateExtensions failed");
        goto Cleanup;
    }
    ...
    if (!SystemTimeToFileTime(&Now, &ExpiredFileTime)) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "SystemTimeToFileTime failed");
        goto Cleanup;
    }
    ...
    if (!FileTimeToSystemTime(&ExpiredFileTime, &Expiration)) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "FileTimeToSystemTime failed");
        goto Cleanup;
    }
...
#pragma prefast(suppress: __WARNING_33088, "Test Only Usage of Self-Signed Certs.")
    ...
    if (NULL == CertContext) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CertCreateSelfSignCertificate failed");
        goto Cleanup;
    }
    ...
    if (!CertSetCertificateContextProperty(
            ...
            &FriendlyNameBlob)) {
        hr = HRESULT_FROM_WIN32(GetLastError());
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", hr, "CertSetCertificateContextProperty failed");
    ...
    )
{
    ...
    if (CertStore != NULL) {
        if (!CertAddCertificateContextToStore(
                ...
                CERT_STORE_ADD_NEW,
                NULL)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", GetLastError(), "CertAddCertificateContextToStore failed");
        }
        CertCloseStore(CertStore, 0);
    }
#endif
...
Done:
    if (Cert != NULL) {
        ...
        if (!CertGetCertificateContextProperty(
                ...
                CertHash,
                &CertHashLength)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", GetLastError(), "CertGetCertificateContextProperty failed");
            CertFreeCertificateContext(Cert);
            Cert = NULL;
        }
    }
