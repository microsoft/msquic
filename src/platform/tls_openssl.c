    ...
    )
{
    ...
    if (Flags != QUIC_SEC_CONFIG_FLAG_CERTIFICATE_FILE) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Flags, "Invalid sec config flags");
    ...
    if (SecurityConfig->SSLCtx == NULL) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_new failed");
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set_ciphersuites failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set1_groups_list failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    Ret =
        SSL_CTX_use_PrivateKey_file(
            ...
            SSL_FILETYPE_PEM);
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_use_PrivateKey_file failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_use_certificate_chain_file failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
    ...
    if (Ret != 1) {
      QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_check_private_key failed");
      Status = QUIC_STATUS_TLS_ERROR;
      goto Exit;
    }
    ...
    )
{
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set_min_proto_version failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set_max_proto_version failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set_default_verify_paths failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
    ...
    if (Ret != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_set_quic_method failed");
        Status = QUIC_STATUS_TLS_ERROR;
        goto Exit;
    }
        ...
        if (QuicOpenSslClientTrustedCert == NULL) {
            ...
            /*Ret =
                SSL_CTX_load_verify_locations(
                    ...
                    NULL);
            if (Ret != 1) {
                QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "SSL_CTX_load_verify_locations failed");
                Status = QUIC_STATUS_TLS_ERROR;
                goto Exit;
            }*/
        }
    ...
    )
{
    ...
    if (WriteKey != NULL) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Status, "QuicTlsDerivePacketProtectionKey failed");
            goto Exit;
        }
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Status, "QuicTlsDerivePacketProtectionIv failed");
            goto Exit;
        }
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Status, "QuicTlsDeriveHeaderProtectionKey failed");
            goto Exit;
        }
    }
    ...
    )
{
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", Status, "QuicTlsUpdateTrafficSecret failed");
        goto Exit;
    }
    ...
    )
{
    ...
    if (OutputBufferLen < PlainTextLen + TagLen) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", OutputBufferLen, "Incorrect output buffer length");
        Ret = -1;
        goto Exit;
    }
    ...
    )
{
    ...
    if (EVP_DecryptInit_ex(CipherCtx, Aead, NULL, NULL, NULL) != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "EVP_DecryptInit_ex failed");
        goto Exit;
    }

    if (EVP_CIPHER_CTX_ctrl(CipherCtx, EVP_CTRL_AEAD_SET_IVLEN, NonceLen, NULL) != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "EVP_CIPHER_CTX_ctrl failed");
        goto Exit;
    }
    ...
    if (AuthData != NULL) {
        if (EVP_DecryptUpdate(CipherCtx, NULL, &Len, AuthData, AuthDataLen) != 1) {
            QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "EVP_DecryptUpdate (AD) failed");
            goto Exit;
        }
    }

    if (EVP_DecryptUpdate(CipherCtx, OutputBuffer, &Len, CipherText, CipherTextLen) != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "EVP_DecryptUpdate (Cipher) failed");
        goto Exit;
    }
    ...
    if (EVP_DecryptFinal_ex(CipherCtx, OutputBuffer + OutLen, &Len) != 1) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %d, %s.", ERR_get_error(), "EVP_DecryptFinal_ex failed");
        goto Exit;
    }
