...
Skip:
    ...
    *NewSocket = Socket;
    ...
    if ((!Socket->UseTcp && !OverrideGlobalQTIPSettings) || IsServerSocket) {
        for (uint16_t i = 0; i < SocketCount; i++) {
    ...
    )
{
    ...
    BOOLEAN NullPerProcSockets = Socket->OverrideUseTcp || (Socket->UseTcp && !Socket->IsServer);
    if (NullPerProcSockets) {
