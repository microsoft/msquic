...
typedef enum CXPLAT_DATAPATH_IO_TYPE {
    CXPLAT_IO_RECV                = 0,
    CXPLAT_IO_SEND                = 1
} CXPLAT_DATAPATH_IO_TYPE;

typedef struct CXPLAT_DATAPATH_INTERNAL_IO_HEADER {
    CXPLAT_DATAPATH_IO_TYPE IoType;
...
QUIC_DISABLED_BY_FUZZER_START;
        ...
        if (Socket->UseRio) {
            ...
            if (SocketProc->RioCq == RIO_INVALID_RQ) {
                int WsaError = WSAGetLastError();
                QuicTraceEvent(
                    DatapathErrorStatus,
                    "[data][%p] ERROR, %u, %s.",
...
QUIC_DISABLED_BY_FUZZER_END;
    ...
    *NewSocket = Socket;
    ...
    for (uint16_t i = 0; i < SocketCount; i++) {
        Status = CxPlatSocketStartReceive(&Socket->Processors[i], NULL, NULL, NULL);
        if (QUIC_FAILED(Status)) {
            goto Error;
        }
    ...
    )
{
    ...
    if (RecvContext != NULL) {
        RecvContext->RioBufferId =
            Datapath->RioDispatch.RIORegisterBuffer(
                RecvContext,
                Datapath->RecvPayloadOffset + MAX_URO_PAYLOAD_LENGTH);
    ...
    )
{
    ...
    if (IoResult == QUIC_STATUS_SUCCESS) {
        ...
        if (QUIC_FAILED(CxPlatSocketStartReceive(AcceptSocketProc, NULL, NULL, NULL))) {
            goto Error;
        }
    ...
    )
{
    ...
    if (IoResult == QUIC_STATUS_SUCCESS) {
        ...
        //
        (void)CxPlatSocketStartReceive(SocketProc, NULL, NULL, NULL);
    ...
    )
{
    ...
    while (SocketProc->RioRecvCount < RIO_RECV_QUEUE_DEPTH) {
        ...
        if (!Datapath->RioDispatch.RIOReceiveEx(
                ...
                &Control, NULL, RioFlags, &RecvContext->IoHeader)) {
            int WsaError = WSAGetLastError();
            QuicTraceEvent(
                DatapathErrorStatus,
                "[data][%p] ERROR, %u, %s.",
    ...
    )
{
    ...
    if (SocketProc->Parent->UseRio) {
        return CxPlatSocketStartRioReceives(SocketProc);
    }
    ...
    )
{
    ...
    if (IoResult == WSAENOTSOCK || IoResult == WSA_OPERATION_ABORTED) {
        ...
        for (WSACMSGHDR *CMsg = WSA_CMSG_FIRSTHDR(&RecvContext->WsaMsgHdr);
            CMsg != NULL;
            CMsg = WSA_CMSG_NXTHDR(&RecvContext->WsaMsgHdr, CMsg)) {
...
void
CxPlatDataPathRestartRioUdpSends(
    _In_ CXPLAT_SOCKET_PROC* SocketProc
    )
{
...
void
CxPlatDataPathRioUdpCompletion(
    _In_ CXPLAT_SOCKET_PROC* SocketProc,
    _In_ ULONG IoResult
    )
{
    ...
    do {
        ...
        for (ULONG i = 0; i < ResultCount; i++) {
            ...
            if (IoHeader->IoType == CXPLAT_IO_RECV) {
                ...
                SocketProc->RioRecvCount--;
            } else if (IoHeader->IoType == CXPLAT_IO_SEND) {
                CXPLAT_RIO_SEND_BUFFER_HEADER* SendHeader =
                    CONTAINING_RECORD(IoHeader, CXPLAT_RIO_SEND_BUFFER_HEADER, IoHeader);
    ...
    CxPlatDataPathRestartUdpReceives(SocketProc);
    CxPlatDataPathRestartRioUdpSends(SocketProc);
    ...
    )
{
    ...
    if (SendData != NULL) {
        SendData->RioBufferId = Datapath->RioDispatch.RIORegisterBuffer(SendData, Size);
    ...
    )
{
    ...
    CXPLAT_DBG_ASSERT(Size + sizeof(*RioHeader) > Size);
    ...
    )
{
    ...
    WSAMhdr.Control.buf = SendData->CtrlBuf;
    ...
    if (Socket->Type == CXPLAT_SOCKET_UDP) {
        if (Socket->UseRio) {
            ...
            Control.Length = WSAMhdr.Control.len;
            ...
            for (UINT8 i = 0; i < SendData->WsaBufferCount; i++) {
                ...
                SendHeader->IoHeader.IoType = CXPLAT_IO_SEND;
                SendHeader->SendData = SendData;

                if (!Datapath->RioDispatch.RIOSendEx(
                        SocketProc->RioRq, &Data, 1, NULL, &RemoteAddr,
                        &Control, NULL, 0, &SendHeader->IoHeader)) {
                    int WsaError = WSAGetLastError();
                    QuicTraceEvent(
                        DatapathErrorStatus,
                        "[data][%p] ERROR, %u, %s.",
    ...
    )
{
    ...
    if (Socket->UseRio) {
        ...
        //
        return CxPlatSocketSendEnqueue(SocketProc, Route, SendData);
    }
