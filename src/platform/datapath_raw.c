    ...
    )
{
    for (uint16_t i = 0; i < PacketCount; i++) {
        ...
        if (Socket) {
            CXPLAT_DBG_ASSERT(!Socket->HasFixedRemoteAddress || Socket->UseTcp == PacketChain->Route->UseQTIP);
...
{
    CXPLAT_DBG_ASSERT(!Socket->HasFixedRemoteAddress || Route->UseQTIP == Socket->UseTcp);
    if (Route->UseQTIP &&
        Socket->Connected &&
