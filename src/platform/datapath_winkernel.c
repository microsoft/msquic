    ...
    )
{
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "WskRegister");
        goto Error;
    }
    WskRegistered = TRUE;
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "WskCaptureProviderNPI");
        goto Error;
    }
    ...
    )
{
    ...
    if (UniHostName.Buffer == NULL) {
        Status = QUIC_STATUS_OUT_OF_MEMORY;
        QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)", "Unicode Hostname", UniHostName.MaximumLength);
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(LibraryErrorStatus, "[ lib] ERROR, %u, %s.", Status, "Convert hostname to unicode");
        goto Error;
    }
...
#pragma prefast(suppress: 28182, "SAL doesn't understand how callbacks work.")
        if (QUIC_FAILED(SocketContext->Irp.IoStatus.Status)) {
            QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", SocketContext->Binding,
    ...
    )
{
    ...
    if (SocketContext->Socket != NULL) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", SocketContext->Binding, Status, "WskCloseSocket");
        }
    }
    ...
    )
{
    ...
    if (Binding == NULL) {
        QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%llu bytes)", "QUIC_DATAPATH_BINDING", sizeof(QUIC_DATAPATH_BINDING));
        Status = QUIC_STATUS_OUT_OF_MEMORY;
        goto Error;
    }
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskSocket");
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskSocket completion");
        goto Error;
    }
    ...
    Status =
        QuicDataPathSetControlSocket(
            ...
            &Option);
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set IPV6_V6ONLY");
        goto Error;
    }
    ...
    Status =
        QuicDataPathSetControlSocket(
            ...
            &Option);
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set IPV6_RECVERR");
        goto Error;
    }
    ...
    Status =
        QuicDataPathSetControlSocket(
            ...
            &Option);
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set IP_RECVERR");
        goto Error;
    }
    ...
    if (Datapath->Features & QUIC_DATAPATH_FEATURE_RECV_COALESCING) {
        ...
        Status =
            QuicDataPathSetControlSocket(
                ...
                &Option);
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set UDP_RECV_MAX_COALESCED_SIZE");
            goto Error;
        }
    }
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskBind");
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskBind completion");
        goto Error;
    }
    ...
    if (RemoteAddress) {
        ...
        Status =
            QuicDataPathSetControlSocket(
                ...
                &MappedRemoteAddress);
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set SIO_WSK_SET_REMOTE_ADDRESS");
            goto Error;
        }
    }
    ...
    *NewBinding = Binding;
    ...
    Status =
        QuicDataPathSetControlSocket(
            ...
            &EventControl);
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "Set SO_WSK_EVENT_CALLBACK");
        goto Error;
    }
    ...
    if (Status == STATUS_PENDING) {
        ...
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskGetLocalAddress");
        goto Error;
    }
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", Binding, Status, "WskGetLocalAddress completion");
        goto Error;
    }
    ...
    )
{
    ...
    QuicTraceEvent(DatapathSendTo, "[ udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst=%!SOCKADDR!",
        Binding,
        SendContext->TotalSize,
        SendContext->WskBufferCount,
        SendContext->SegmentSize,
    ...
    if (QUIC_FAILED(Status)) {
        QuicTraceEvent(DatapathErrorStatus, "[ udp][%p] ERROR, %u, %s.", SocketContext->Binding, Status, "WskSendMessages");
        //
    ...
    )
{
    ...
    QuicTraceEvent(DatapathSendFromTo, "[ udp][%p] Send %u bytes in %hhu buffers (segment=%hu) Dst=%!SOCKADDR!, Src=%!SOCKADDR!",
        Binding,
        SendContext->TotalSize,
        SendContext->WskBufferCount,
        SendContext->SegmentSize,
