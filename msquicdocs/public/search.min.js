import{a,b as R,c as q,d as L,e as P,j as w}from"./chunk-LSENCIDT.min.js";import"./chunk-MY7PHHTQ.min.js";var i;async function T(){let s=document.getElementById("search-query");if(!s||!window.Worker)return;let m=L("docfx:rel")||"",g=new Worker(m+"public/search-worker.min.js",{type:"module"});g.onerror=e=>{console.error("Error occurred at search-worker. message: "+e.message)},g.onmessage=function(e){switch(e.data.e){case"index-ready":s.disabled=!1,s.addEventListener("input",$),s.addEventListener("keypress",function(t){t.key==="Enter"&&event.preventDefault()}),window.docfx.searchReady=!0;break;case"query-ready":document.body.setAttribute("data-search","true"),l(e.data.d,0),window.docfx.searchResultReady=!0,s.value===""&&document.body.removeAttribute("data-search");break}};let{lunrLanguages:p}=await q();g.postMessage({init:{lunrLanguages:p}});function $(){if(i=s.value,i==="")document.body.removeAttribute("data-search");else{let e=i.replace(/\s+/g," ").split(" ").map(t=>"+"+t).join(" ");g.postMessage({q:e})}}function u(e,t){let h=e.split(/\/+/),r=t.split(/\/+/),b=h.length-1,v=[];for(let c=0;c<r.length;c++)r[c]===".."?b--:r[c]!=="."&&v.push(r[c]);return h.slice(0,b).concat(v).join("/")}function d(e){let h=i.split(/\s+/g),r=e.indexOf(h[0]);if(r>512)return"..."+e.slice(r-512,r+512)+"...";if(r<=512)return e.slice(0,r+512)+"..."}function l(e,t){let r=Math.ceil(e.length/10);R(b(t),document.getElementById("search-results"));function b(c){if(e.length===0)return a`<div class="search-list">${P("searchNoResults",{query:i})}</div>`;let f=c*10,k=e.slice(f,f+10),y=a`
        <div class="search-list">${P("searchResultsCount",{count:e.length.toString(),query:i})}</div>
        <div class="sr-items">${k.map(o=>{let n=window.location.href,M=u(n,m+o.href),O=m+o.href+"?q="+i,E=o.summary?d(o.summary):"";return a`
            <div class="sr-item">
              <div class="item-title"><a href="${O}" target="_blank" rel="noopener noreferrer">${x(o.title,i)}</a></div>
              <div class="item-href">${x(M,i)}</div>
              <div class="item-brief">${x(E,i)}</div>
            </div>`})}
        </div>`;return a`${y} ${v()}`}function v(){let f=Math.max(0,Math.min(t-2,r-5)),k=Math.min(r,f+5),y=Array.from(new Array(k-f).keys()).map(n=>n+f);if(y.length<=1)return null;return a`
        <nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link ${w({disabled:t<=0})}" href="#" aria-label="Previous"
                @click="${()=>o(t-1)}">
            ...
            ${y.map(n=>a`
              <li class="page-item">
                <a class="page-link ${w({active:t===n})}" href="#"
                  @click="${()=>o(n)}">${n+1}</a></li>`)}
            <li class="page-item">
              <a class="page-link ${w({disabled:t>=r-1})}" href="#" aria-label="Next"
                @click="${()=>o(t+1)}">
                <span aria-hidden="true">&raquo;</span>
              </a>
            </li>
          </ul>
        </nav>`;function o(n){n>=0&&n<r&&l(e,n)}}}}function x(s,m){let p=m.split(/\s+/g).map(l=>l.toLowerCase()),$=s.toLowerCase(),u=[],d=0;for(let l=0;l<p.length;l++){let e=p[l],t=$.indexOf(e,d);t>=0&&(u.push(a`${s.slice(d,t)}`),u.push(a`<b>${s.slice(t,t+e.length)}</b>`),d=t+e.length)}return u.push(a`${s.slice(d)}`),a`${u}`}export{T as enableSearch};
...
// SIG // +rgYWsSsS73LaQQz7I+eBEliCCDpm5zuwLfRGVHqLzug
...
// SIG // BgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBC