...
.DESCRIPTION
    This script parses Cobertura XML coverage reports generated by OpenCppCoverage
    and compares line coverage between baseline (pre-PR) and current (post-PR) versions.
    
    Note: OpenCppCoverage only supports line coverage, not branch coverage.
    Branch coverage fields in the output will show 0 or N/A.
...
function Format-CoverageReport {
    ...
    # Note about branch coverage
    [void]$sb.AppendLine("> ℹ️ *Note: OpenCppCoverage only supports line coverage. Branch coverage is not available.*")
    [void]$sb.AppendLine("")
