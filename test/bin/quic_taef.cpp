    ...
    )
{
    ...
    LogError("[test]File: %s, Function: %s, Line: %d", File, Function, Line);
    LogError("[test]FAIL: %s", Buffer);
...
QuicTestDriver::Initialize() {
    ...
    if (ScmHandle == nullptr) {
        Error = GetLastError();
        LogError("[test] GetFullPathName failed, 0x%x.", Error);
        return Error;
    }
QueryService:
    ServiceHandle =
    ...
    if (ServiceHandle == nullptr) {
        LogError("[test] OpenService failed, 0x%x.", GetLastError());
        ...
        if (ServiceHandle == nullptr) {
            ...
            LogError("[test] CreateService failed, 0x%x.", Error);
            return Error;
        }
    }
...
QuicTestDriver::Start() {
    if (!StartService(ServiceHandle, 0, nullptr)) {
        ...
        if (Error != ERROR_SERVICE_ALREADY_RUNNING) {
            LogError("[test] StartService failed, 0x%x.", Error);
            return Error;
        }
    }
    return ERROR_SUCCESS;
    ...
    )
{
    ...
    if (DeviceHandle == INVALID_HANDLE_VALUE) {
        Error = GetLastError();
        LogError("[test] CreateFile failed, 0x%x.", Error);
        return Error;
    }
    ...
    if (Error != NO_ERROR) {
        ...
        Error = GetLastError();
        LogError("[test] SendIOCTL(IOCTL_QUIC_SEC_CONFIG) failed, 0x%x.", Error);
        return Error;
    }
    Initialized = true;
    return ERROR_SUCCESS;
    ...
    )
{
    ...
    if (Overlapped.hEvent == nullptr) {
        Error = GetLastError();
        LogError("[test] CreateEvent failed, 0x%x.", Error);
        return Error;
    }
    LogVerbose("[test] Sending IOCTL %u with %u bytes.",
    ...
    if (!DeviceIoControl(
        ...
        if (Error != ERROR_IO_PENDING) {
            ...
            LogError("[test] DeviceIoControl failed, 0x%x.", Error);
            return Error;
        }
    }
    ...
    if (!GetOverlappedResultEx(
        ...
        LogError("[test] GetOverlappedResultEx failed, 0x%x.", Error);
    } else {
        Error = ERROR_SUCCESS;
    }
    CloseHandle(Overlapped.hEvent);
    ...
    )
{
    ...
    if (QUIC_FAILED(Status)) {
        LogError("[test] GetSecConfigComplete failed, 0x%x.", Status);
    }
...
MODULE_SETUP(GlobalTestSetup) {
    ...
    if ((SelfSignedCertParams =
        ...
        LogError("[test] QuicPlatGetSelfSignedCert failed.");
        return false;
    }
    ...
    if (KernelMode == 1) {
        ...
        if (Error != NO_ERROR) {
            LogError("[test] TestDriver.Initialize failed, 0x%x.", Error);
            QuicPlatFreeSelfSignedCert(SelfSignedCertParams);
            return false;
        }
        Error = TestDriver.Start();
        if (Error != NO_ERROR) {
            LogError("[test] TestDriver.Start failed, 0x%x.", Error);
        ...
        if (Error != ERROR_SUCCESS) {
            LogError("[test] TestClient.Initialize failed, 0x%x.", Error);
            TestDriver.Uninitialize();
            QuicPlatFreeSelfSignedCert(SelfSignedCertParams);
            return false;
        }
        ...
        if (!CompartmentHelper::CreateCompartment(TestCompartmentID)) {
            LogError("[test] CreateCompartment failed.");
        ...
        if (QUIC_FAILED(MsQuicOpenV1(&MsQuic))) {
            LogError("[test] MsQuicOpen failed.");
            QuicPlatFreeSelfSignedCert(SelfSignedCertParams);
            return false;
        }
        if (QUIC_FAILED(MsQuic->RegistrationOpen("MsQuicBVT", &Registration))) {
            LogError("[test] RegistrationOpen failed.");
            MsQuicClose(MsQuic);
            QuicPlatFreeSelfSignedCert(SelfSignedCertParams);
            return false;
        }
        if (!LoadSecConfig()) {
            LogError("[test] Failed to load the security config.");
