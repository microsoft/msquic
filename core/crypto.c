    ...
    if (Frame->Length == 0) {
        ...
        if (KeyType != Crypto->TlsState.ReadKey) {
            if (QuicRecvBufferAlreadyReadData(
                ...
                QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "Tried crypto with incorrect key.");
        ...
        if (QUIC_FAILED(Status)) {
            if (Status == QUIC_STATUS_BUFFER_TOO_SMALL) {
                QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "Tried to write beyond crypto flow control limit.");
    ...
    )
{
    ...
    if (ResultFlags & QUIC_TLS_RESULT_ERROR) {
        QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Crypto->TlsState.AlertCode, "Received alert from TLS.");
    ...
    if (ResultFlags & QUIC_TLS_RESULT_READ_KEY_UPDATED) {
        ...
        if (QuicRecvBufferHasUnreadData(&Crypto->RecvBuffer)) {
            QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "Leftover crypto data in previous encryption level.");
            QuicConnTransportError(Connection, QUIC_ERROR_PROTOCOL_VIOLATION);
            return;
        }
    ...
    if (IsClientInitial) {
        ...
        if (BufferOffset == 0 &&
            ...
            if (AcceptResult != QUIC_CONNECTION_ACCEPT) {
                QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, AcceptResult, "Connection rejected.");
    ...
    if (*NewReadKey == NULL) {
        ...
        Status =
            QuicPacketKeyUpdate(
                ...
                NewReadKey);
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Status, "Failed to update read packet key.");
            goto Error;
        }
        ...
        Status =
            QuicPacketKeyUpdate(
                ...
                NewWriteKey);
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Status, "Failed to update write packet key");
            goto Error;
        }
    }
