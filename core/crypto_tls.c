    ...
    )
{
    ...
    if (TPLen < sizeof(uint16_t)) {
        QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "Invalid length for QUIC TP param list length");
        goto Exit;
    }
    ...
    if (Offset + TPParamListLen > TPLen) {
        QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "QUIC TP param list length too large");
        goto Exit;
    }
    ...
    while (Offset < TPLen) {
        ...
        if (Offset + TLS_HDR_SIZE > TPLen) {
            QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "QUIC TP params invalid leftover length");
            goto Exit;
        }
        ...
        if (Id <= QUIC_TP_ID_MAX) {
            ...
            if (ParamsPresent & (1 << Id)) {
                QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Id, "Duplicate QUIC TP type");
                goto Exit;
            }
        ...
        if (Offset + Length > TPLen) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Id, "QUIC TP value length too long");
            goto Exit;
        }
    ...
    #define TRY_READ_VAR_INT(Param) \
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_BIDI_LOCAL:
            ...
            if (TransportParams->InitialMaxStreamDataBidiLocal > QUIC_TP_MAX_MAX_STREAMS) {
                QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "TP InitialMaxStreamDataBidiLocal too big");
                goto Exit;
            }
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_BIDI_REMOTE:
            ...
            if (TransportParams->InitialMaxStreamDataBidiRemote > QUIC_TP_MAX_MAX_STREAMS) {
                QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "TP InitialMaxStreamDataBidiRemote too big");
                goto Exit;
            }
        ...
        case QUIC_TP_ID_INITIAL_MAX_STREAM_DATA_UNI:
            ...
            if (TransportParams->InitialMaxStreamDataUni > QUIC_TP_MAX_MAX_STREAMS) {
                QuicTraceEvent(ConnError, "[conn][%p] ERROR, %s.", Connection, "TP InitialMaxStreamDataUni too big");
                goto Exit;
            }
