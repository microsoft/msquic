    ...
    )
{
    ...
    QuicTraceEvent(ApiEnter, "[ api] Enter %d (%p).", QUIC_TRACE_API_LISTENER_OPEN, SessionHandle);
    ...
    )
{
    ...
    QuicTraceEvent(ApiEnter, "[ api] Enter %d (%p).", QUIC_TRACE_API_LISTENER_CLOSE, Handle);
    ...
    )
{
    ...
    QuicTraceEvent(ApiEnter, "[ api] Enter %d (%p).", QUIC_TRACE_API_LISTENER_START, Handle);
...
#pragma prefast(suppress: __WARNING_25024, "Pointer cast already validated.")
    ...
    QuicTraceEvent(ListenerStarted, "[list][%p] Started, Binding=%p, LocalAddr=%p, CLOGBUG=%c", Listener, Listener->Binding, (uint8_t*)&Listener->LocalAddress, LOG_ADDR_LEN(Listener->LocalAddress));
...
{
    QuicTraceEvent(ApiEnter, "[ api] Enter %d (%p).", QUIC_TRACE_API_LISTENER_STOP, Handle);
