    ...
    )
{
    ...
    QuicTraceEvent(ConnCreated, "[conn][%p] Created, IsServer=%d, CLOGBUG=%I", Connection, IsServer, Connection->Stats.CorrelationId);
    ...
    if (IsServer) {
        ...
        QuicTraceEvent(ConnLocalAddrAdded, "[conn][%p] New Local IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Path->LocalAddress, LOG_ADDR_LEN(Path->LocalAddress));

        Path->RemoteAddress = Datagram->Tuple->RemoteAddress;
        Connection->State.RemoteAddressSet = TRUE;
        QuicTraceEvent(ConnRemoteAddrAdded, "[conn][%p] New Remote IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Path->RemoteAddress, LOG_ADDR_LEN(Path->RemoteAddress));
        ...
        QuicTraceEvent(ConnDestCidAdded, "[conn][%p] New Destination CID: %p, CLOGBUG=%c", Connection, Path->DestCid->CID.Data, Path->DestCid->CID.Length);
        ...
        QuicTraceEvent(ConnSourceCidAdded, "[conn][%p] New Source CID: %p, CLOGBUG=%c", Connection, SourceCID->CID.Data, SourceCID->CID.Length);
...
{
    QuicTraceEvent(ConnRundown, "[conn][%p] Rundown, IsServer=%d, CLOGBUG=%I", Connection, QuicConnIsServer(Connection), Connection->Stats.CorrelationId);
    ...
    if (Connection->State.Started) {
        for (uint8_t i = 0; i < Connection->PathsCount; ++i) {
            if (Connection->State.LocalAddressSet || i != 0) {
                QuicTraceEvent(ConnLocalAddrAdded, "[conn][%p] New Local IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Connection->Paths[i].LocalAddress, LOG_ADDR_LEN(Connection->Paths[i].LocalAddress));
            }
            if (Connection->State.RemoteAddressSet || i != 0) {
                QuicTraceEvent(ConnRemoteAddrAdded, "[conn][%p] New Remote IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Connection->Paths[i].RemoteAddress, LOG_ADDR_LEN(Connection->Paths[i].RemoteAddress));
            }
        }
        ...
        for (QUIC_LIST_ENTRY* Entry = Connection->DestCIDs.Flink;
            ...
            const QUIC_CID_QUIC_LIST_ENTRY* DestCID =
                QUIC_CONTAINING_RECORD(
                    ...
                    QUIC_CID_QUIC_LIST_ENTRY,
                    Link);
            QuicTraceEvent(ConnDestCidAdded, "[conn][%p] New Destination CID: %p, CLOGBUG=%c", Connection, DestCID->CID.Data, DestCID->CID.Length);
        }
    }
    if (Connection->State.Connected) {
        QuicConnOnQuicVersionSet(Connection);
    ...
    )
{
    ...
    QuicTraceEvent(ConnDestCidRemoved, "[conn][%p] Removed Destination CID: %p, CLOGBUG=%c", Connection, Path->DestCid->CID.Data, Path->DestCid->CID.Length);
    Connection->DestCIDCount--;
    ...
    )
{
    ...
    QuicTraceEvent(ConnShutdownComplete, "[conn][%p] Shutdown complete, PeerFailedToAcknowledged=%c.", Connection, Connection->State.ShutdownCompleteTimedOut);
    ...
    )
{
    ...
    if (IsFirstCloseForConnection) {
        ...
        if (Flags & QUIC_CLOSE_APPLICATION) {
            QuicTraceEvent(ConnAppShutdown, "[conn][%p] App Shutdown: %I (Remote=%c)", Connection, ErrorCode, ClosedRemotely);
        } else {
            QuicTraceEvent(ConnTransportShutdown, "[conn][%p] Transport Shutdown: %I (Remote=%c) (QS=%c)", Connection, ErrorCode, ClosedRemotely, !!(Flags & QUIC_CLOSE_QUIC_STATUS));
        }
    ...
    )
{
    ...
    if (QuicConnIsServer(Connection)) {
        ...
        if (QUIC_FAILED(Status)) {
            QuicTraceEvent(ConnErrorStatus, "[conn][%p] ERROR, %d, %s.", Connection, Status, "QuicBindingGenerateStatelessResetToken");
            goto Error;
        }
    ...
    )
{
    ...
    if (Packet->SourceCIDLen != DestCID->CID.Length ||
        ...
        QuicTraceEvent(ConnDestCidRemoved, "[conn][%p] Removed Destination CID: %p, CLOGBUG=%c", Connection, DestCID->CID.Data, DestCID->CID.Length);
    ...
    )
{
    ...
    if (!Packet->IsShortHeader) {
        ...
        if (!Path->IsPeerValidated && Packet->ValidToken) {
            ...
            if (Connection->OrigCID == NULL) {
                QuicTraceEvent(AllocFailure, "Allocation of '%s' failed. (%I bytes)", "OrigCID", sizeof(QUIC_CID) + Token.OrigConnIdLength);
                return FALSE;
            }
    ...
    )
{
    ...
    QuicTraceEvent(ConnPacketRecv, "[conn][%p][RX][%I] %c (%hd bytes)", Connection, Packet->PacketNumber, Packet->IsShortHeader ? QUIC_TRACE_PACKET_ONE_RTT : (Packet->LH->Type + 1), Packet->HeaderLength + Packet->PayloadLength);
    ...
    while (Offset < PayloadLength) {
        ...
        case QUIC_FRAME_RETIRE_CONNECTION_ID: {
            ...
            if (SourceCid != NULL) {
                ...
                QuicTraceEvent(ConnDestCidRemoved, "[conn][%p] Removed Destination CID: %p, CLOGBUG=%c", Connection, SourceCid->CID.Data, SourceCid->CID.Length);
                QUIC_FREE(SourceCid);
                if (IsLastCid) {
...
Done:
    ...
    if (Connection->State.HandleShutdown || Connection->State.HandleClosed) {
        QuicTraceLogVerbose("[%c][RX][%llu] not acked (connection is closed)",
            ...
            if (SourceCid->CID.IsInitial) {
                if (QuicConnIsServer(Connection) && SourceCid->Link.Next != NULL) {
                    ...
                    if (NextSourceCid->CID.IsInitial) {
                        ...
                        QuicTraceEvent(ConnDestCidRemoved, "[conn][%p] Removed Destination CID: %p, CLOGBUG=%c", Connection, NextSourceCid->CID.Data, NextSourceCid->CID.Length);
    ...
    while ((Datagram = DatagramChain) != NULL) {
        ...
        if (PacketPath != Path) {
            if (BatchCount != 0) {
                ...
                ResetIdleTimeout |=
                    QuicConnRecvBatch(Connection, Path, BatchCount, Batch, Cipher);
                BatchCount = 0;
            }
    ...
    )
{
    ...
    case QUIC_PARAM_CONN_LOCAL_ADDRESS: {
        ...
        QuicTraceEvent(ConnLocalAddrAdded, "[conn][%p] New Local IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Connection->Paths[0].LocalAddress, LOG_ADDR_LEN(Connection->Paths[0].LocalAddress));
        ...
        if (Connection->State.Started) {
            ...
            QuicTraceEvent(ConnLocalAddrRemoved, "[conn][%p] Removed Local IP: %p, CLOGBUG=%c", Connection, (const uint8_t*)&Connection->Paths[0].LocalAddress, LOG_ADDR_LEN(Connection->Paths[0].LocalAddress));
