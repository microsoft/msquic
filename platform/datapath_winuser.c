    ...
    )
{
    ...
    if (IoResult == WSAENOTSOCK || IoResult == WSA_OPERATION_ABORTED) {
        ...
        QuicTraceEvent(DatapathRecv, "[ udp][%p] Recv %d bytes (segment=%hd) Src=%c Dst=%c, CLOGBUG=%p, CLOGBUG=%p", SocketContext->Binding, NumberOfBytesTransferred, MessageLength, LOG_ADDR_LEN(*LocalAddr), LOG_ADDR_LEN(*RemoteAddr), (UINT8*)LocalAddr, (UINT8*)RemoteAddr);
    ...
    )
{
    ...
    QuicTraceEvent(DatapathSendTo, "[ udp][%p] Send %d bytes in %c buffers (segment=%hd) Dst=%p, CLOGBUG=%c", Binding, SendContext->TotalSize, SendContext->WsaBufferCount, SendContext->SegmentSize, (UINT8*)RemoteAddress, LOG_ADDR_LEN(*RemoteAddress));
    ...
    )
{
    ...
    QuicTraceEvent(DatapathSendFromTo, "[ udp][%p] Send %d bytes in %c buffers (segment=%hd) Src=%p Dst=%p, CLOGBUG=%c, CLOGBUG=%c", Binding, SendContext->TotalSize, SendContext->WsaBufferCount, SendContext->SegmentSize, (UINT8*)LocalAddress, (UINT8*)RemoteAddress, LOG_ADDR_LEN(*RemoteAddress), LOG_ADDR_LEN(*LocalAddress));
