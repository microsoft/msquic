    ...
    )
{
    ...
    if (RemoteAddr->si_family == AF_INET) {
        QuicTraceLogVerbose("[sock][%p] Received [%zd] (buflen=[%h]) bytes Src=[%s:%h] Dst=[%s:%h], bind=[%p].",
        ...
        QuicTraceLogVerbose("[sock][%p] Received [%zd] (buflen=[%h]) bytes Src=[%s:%h] Dst=[%s:%h%%%" PRIu32 "], bind=[%p].",
            SocketContext, BytesTransferred,
            RecvPacket->BufferLength,
    ...
    )
{
    ...
    for (size_t i = 0; i < SendContext->BufferCount; ++i) {
        if (RemoteAddress->si_family == AF_INET) {
            QuicTraceLogVerbose("[sock][%p] SocketFd=[%d], sending %" PRIu32 " bytes Dst=[%s:%h] (%p)",
                SocketContext,
                SocketContext->SocketFd,
                SendContext->Buffers[i].Length,
    ...
    )
{
    ...
    for (size_t i = 0; i < SendContext->BufferCount; ++i) {
        if (RemoteAddress->si_family == AF_INET) {
            QuicTraceLogVerbose("[sock][%p] SocketFd=[%d], sending %" PRIu32 " bytes Src=[%s:%h%%%" PRIu32 "] Dst=[%s:%h] (%p)",
                SocketContext,
                SocketContext->SocketFd,
                SendContext->Buffers[i].Length,
