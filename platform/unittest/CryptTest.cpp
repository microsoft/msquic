    ...
    )
{
    ...
    Log::Comment(String().Format(L"%S[%u]: %S", Name, (uint32_t)Length, Str));
    delete [] Str;
}

struct CryptTest : public WEX::TestClass<CryptTest>
{
    CryptTest() { }
    ~CryptTest() { }

    BEGIN_TEST_CLASS(CryptTest)
    END_TEST_CLASS()
    ...
    struct QuicKey
    {
        ...
        QuicKey(QUIC_AEAD_TYPE AeadType, const uint8_t* const RawKey) : Ptr(NULL) {
            ...
            if (Status == QUIC_STATUS_NOT_SUPPORTED) {
                Log::Comment(L"AEAD Type unsupported");
                return;
            }
    ...
    struct QuicHash
    {
        ...
        QuicHash(
            ...
            if (Status == QUIC_STATUS_NOT_SUPPORTED) {
                Log::Comment(L"HASH Type unsupported");
                return;
            }
            VERIFY_QUIC_SUCCESS(Status);
            VERIFY_IS_NOT_NULL(Ptr);
    ...
    TEST_METHOD(WellKnownClientInitial)
    {
        ...
        VERIFY_ARE_EQUAL(0, memcmp(ExpectedSample.Data, PacketBuffer + InitialPacketHeader.Length, ExpectedSample.Length));
        ...
        VERIFY_ARE_EQUAL(0, memcmp(ExpectedHpMask.Data, HpMask, ExpectedHpMask.Length));
        ...
        VERIFY_ARE_EQUAL(0, memcmp(ExpectedHeader.Data, PacketBuffer, ExpectedHeader.Length));
        ...
        VERIFY_ARE_EQUAL(EncryptedPacket.Length, (uint16_t)sizeof(PacketBuffer));
        VERIFY_ARE_EQUAL(0, memcmp(EncryptedPacket.Data, PacketBuffer, EncryptedPacket.Length));
    ...
    TEST_METHOD(Encryption)
    {
        BEGIN_TEST_METHOD_PROPERTIES()
            TEST_METHOD_PROPERTY(L"Data:AEAD", L"{0,1,2}")
        END_TEST_METHOD_PROPERTIES()

        int AEAD;
        TestData::TryGetValue(L"AEAD", AEAD);
        ...
        VERIFY_IS_TRUE(Key.Encrypt(Iv, 0, NULL, sizeof(Buffer), Buffer));
        VERIFY_IS_TRUE(Key.Decrypt(Iv, 0, NULL, sizeof(Buffer), Buffer));
        VERIFY_IS_TRUE(Key.Encrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
        VERIFY_IS_TRUE(Key.Decrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
        ...
        VERIFY_IS_TRUE(Key.Encrypt(Iv, 0, NULL, sizeof(Buffer), Buffer));
        VERIFY_IS_FALSE(Key.Decrypt(Iv, 0, NULL, sizeof(Buffer) - 1, Buffer));

        VERIFY_IS_TRUE(Key.Encrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
        VERIFY_IS_FALSE(Key.Decrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer) - 1, Buffer));

        VERIFY_IS_TRUE(Key.Encrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
        VERIFY_IS_FALSE(Key.Decrypt(Iv, sizeof(AuthData) - 1, AuthData, sizeof(Buffer), Buffer));

        VERIFY_IS_TRUE(Key.Encrypt(Iv, 0, NULL, sizeof(Buffer), Buffer));
        Buffer[0] ^= 1;
        VERIFY_IS_FALSE(Key.Decrypt(Iv, 0, NULL, sizeof(Buffer), Buffer));
        ...
        VERIFY_IS_TRUE(Key.Encrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
        AuthData[0] ^= 1;
        VERIFY_IS_FALSE(Key.Decrypt(Iv, sizeof(AuthData), AuthData, sizeof(Buffer), Buffer));
    ...
    TEST_METHOD(HashWellKnown)
    {
        BEGIN_TEST_METHOD_PROPERTIES()
            TEST_METHOD_PROPERTY(L"Data:HASH", L"{0,1,2}")
        END_TEST_METHOD_PROPERTIES()

        int HASH;
        TestData::TryGetValue(L"HASH", HASH);
        ...
        VERIFY_IS_TRUE(
            Hash.Compute(
                ...
                OutputLength,
                Output));
        VERIFY_ARE_EQUAL((uint16_t)WellKnownOutput[HASH]->Length, OutputLength);
        VERIFY_ARE_EQUAL(0, memcmp(WellKnownOutput[HASH]->Data, Output, OutputLength));
    }

    TEST_METHOD(HashRandom)
    {
        BEGIN_TEST_METHOD_PROPERTIES()
            TEST_METHOD_PROPERTY(L"Data:HASH", L"{0,1,2}")
        END_TEST_METHOD_PROPERTIES()
        ...
        VERIFY_IS_TRUE(
            Hash.Compute(
                ...
                OutputLength,
                Output2));
        VERIFY_ARE_EQUAL(0, memcmp(Output, Output2, OutputLength));
    }
};
